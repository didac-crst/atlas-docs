...
def test_v03_migration_backfills_external_references_and_entity_targets(
    ...
    inverse_type_id = str(uuid.uuid4())
    ...
    try:
        ...
        with engine.begin() as conn:
            # Simulate a v0.3 companion edge that cannot exist in v0.1.
            conn.execute(
                text(
                    """
                    INSERT INTO relationship_types (
                        ...
                        :id, 'answered-by', 'Answered By', NULL, 'directed', NULL
                    )
                    """
                ),
                {"id": inverse_type_id},
        ...
        # and drops concept-sourced companion edges that v0.1 cannot represent.
        ...
        with engine.connect() as conn:
            ...
            remaining = conn.execute(text("SELECT id FROM relationships")).fetchall()
            assert remaining == [(relationship_id,)]
    finally:
        engine.dispose()
